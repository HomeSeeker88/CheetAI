...
from src.Models.Summaries.Summaries import LinearRegressionSummary
...
class LinearRegression(Model):
    ...
    def fit(
        self, x_values: np.ndarray | list[np.ndarray], y_values: np.ndarray
    ) -> None:
        ...
        if isinstance(x_values, list):
            ...
            self.coefficients = np.matmul(
                np.matmul(np.linalg.inv(np.matmul(x_values_t, x_values)), x_values_t),
                y_values,
            )
        else:
    ...
    def _calculate_root_mean_squared_error(
        self, y_pred_values: np.ndarray
    ) -> np.float64:
        return np.sqrt(self._calculate_mean_square_error(y_pred_values=y_pred_values))

    def _calculate_residual_sum_of_squares(
        self, y_pred_values: np.ndarray
    ) -> np.float64:
        if len(self.train_y) != len(y_pred_values):
            raise ValueError("Vectors must be the same size!")
    ...
    def _calculate_r_square(self, y_pred_values: np.ndarray) -> np.float64:
        return (
            1
            - self._calculate_residual_sum_of_squares(y_pred_values=y_pred_values)
            / self._calculate_total_sum_of_squares()
        )

    def _check_error_normality(self, y_pred_values: np.ndarray):
        return (
            True if shapiro(np.abs(self.train_y - y_pred_values))[1] > 0.05 else False
        )

    def evaluate(self, y_pred_values: np.ndarray):
        self.summary = LinearRegressionSummary(
            rmse=self._calculate_total_sum_of_squares(),
    ...
    def dry_plot(
        ...
        for x, y, y_pred in zip(x_values, y_values, y_pred_values):
            ax.vlines(
                x=x, ymin=min(y_pred, y), ymax=max(y_pred, y), color="red", ls="--"
            )
